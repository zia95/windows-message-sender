...
namespace WinMessageSenderCS
{
    public static class FunctionImports
    {
        ...
        */
        ...
        [DllImport("user32.dll")]
        public static extern int PostMessageW(IntPtr hWnd, int Msg, IntPtr wParam, IntPtr lParam);//its int = bool

        [DllImport("user32.dll")]
        public static extern int PostMessageA(IntPtr hWnd, int Msg, IntPtr wParam, IntPtr lParam);//its int = bool
        ...
        public static extern uint GetProcessImageFileNameW(IntPtr hProcess, StringBuilder lpImageFileName, int nSize);
        ...
        public enum SMMethod { Send, PostA, PostW  }
        public static int SendMessage(IntPtr hWnd, int Msg, IntPtr wParam, IntPtr lParam, SMMethod method)
        {
            switch(method)
            {
                case SMMethod.Send:
        ...
        public static string GetProcessNameById(uint dwProcessId)
        {
            if (dwProcessId <= 0)
                return null;

            IntPtr hProc = OpenProcess(PROCESS_QUERY_INFORMATION, false, dwProcessId);
            if (hProc != IntPtr.Zero)
            {
                StringBuilder outText = new StringBuilder(260);

                uint nWritten = GetProcessImageFileNameW(hProc, outText, outText.Capacity);

                CloseHandle(hProc);

                return nWritten != 0 ? outText.ToString() : null;
            }
            return null;
        }

        public static uint GetProcessId(IntPtr hWnd) => GetWindowThreadProcessId(hWnd, IntPtr.Zero);

        public static string GetWindowText(IntPtr hWnd)
        {
            int textLength = GetWindowTextLengthW(hWnd);
            StringBuilder outText = new StringBuilder(textLength + 1);
            if (GetWindowTextW(hWnd, outText, outText.Capacity) != 0)
        ...
        public static string GetClassName(IntPtr hWnd)
        {
            ...
            if (GetClassNameW(hWnd, outText, outText.Capacity) != 0)
                return outText.ToString();
            return null;
        }

        public static List<IntPtr> GetAllParentWindow()
        {
            List<IntPtr> parentWnds = new List<IntPtr>();
        ...
        public static List<IntPtr> GetAllChildWindows(IntPtr hWndParent)
        {
